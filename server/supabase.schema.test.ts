...
describe("LinhFarm Supabase schema", () => {
  it("contains the required tables, seed data, RLS policies and order RPC", () => {
    for (const table of ["products", "orders", "order_items", "suppliers", "purchase_orders", "purchase_order_items", "store_settings"]) {
    ...
    expect(schema).toContain("create or replace function public.create_purchase_order_with_items");
